...
| **Show in frontend** | On by default. Lists the product in shop and catalog pages. Turning it off hides it from `getProducts()` only, it still sells everywhere it is used. See [Storefront visibility](/backend-template-engine/products-and-shipping#storefront-visibility) |
...
| Buyer's country | Rate applied |
|-----------------|--------------|
| Matches a per-country override | That override's cost |
| `US` | **Mainland US Cost** |
| Anything else | **International Cost** |

<Warning>
**The two rates are independent - neither one falls back to the other.** Leaving a field empty does not mean "charge the other rate". It means **free shipping for that region**, applied silently with no warning.

A profile set to Mainland US `$10` with International left blank charges $10 in the US and **$0.00 in every other country**. The checkout's Shipping line renders `$0.00` and the order is placed with no shipping charge at all.

This is easiest to hit while previewing your own checkout from outside the US: the country field defaults to the visitor's geolocated country, so the page shows `$0.00` shipping and looks broken even though the US rate is configured correctly. Switch the country to **United States** to see the US rate apply.

Fill in **both** fields on every profile. If you truly do not ship outside the US, restrict which countries the checkout will accept via `checkout_settings.countries` (see [Countries and US states allowlist](/checkouts/checkout-functionality-reference)) instead of leaving International blank.
</Warning>
...
| `show_in_frontend` | `boolean` | `true` when the product is listed in shop and catalog pages. Does not affect whether it can be sold |
...
Returns the brand's products, with optional filters. Products with **Show in frontend** off are left out unless you ask for them, see [Storefront visibility](/backend-template-engine/products-and-shipping#storefront-visibility).
...
| `show_in_frontend` | `boolean \| string` | Defaults to `true`. Pass `'any'` for everything, `false` for hidden products only |
