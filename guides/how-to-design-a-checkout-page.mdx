...
## Prerequisites
{/* SCREENSHOT: Prerequisites · relevant screen or example output · desktop */}
...
<Note>
**Checkout submission is formless.** The examples below wrap fields in a `<form>` for autofill and semantics, but the runtime never submits it: the checkout script sets its form reference to `null` and collects every named / `data-template-value` field from the page wrapper instead. Submission is a **click** on `[data-checkout-submit]` (or `.checkout-button`) that calls `preventDefault()` and posts via fetch. So `method="POST"` and `form="checkout-form"` have no effect, and fields do not have to live inside the `<form>` element to be submitted.
</Note>

---

## Example 1: Standard checkout
{/* SCREENSHOT: Example 1: Standard checkout · relevant screen or example output · desktop */}
...
<form id="checkout-form" method="POST">
  ...
  <template-if data-condition="checkout.shipping_same_as_billing == false">
    <section class="billing-info-card">
        ...
        <div class="form-grid">
          ...
          <div class="floating-label-group">
            ...
            <input type="text" name="billing_last_name"
                   data-template-value="checkout.customer.billing_last_name" class="form-input" />
            <div class="form-error" data-checkout-error="billing_last_name"></div>
          </div>
          <div class="floating-label-group">
            <label class="form-label">Street Address *</label>
      ...
      <template-else>
        ...
        <input type="hidden" name="billing_last_name" data-template-value="checkout.customer.billing_last_name" />
  ...
  <!-- Submit -->
  <button type="button" data-checkout-submit class="pay-now-button">
...
{/* SCREENSHOT: Example 2: Wallet-first checkout · relevant screen or example output · desktop */}
...
<form id="checkout-form" method="POST">
  ...
  <!-- Wallet buttons at the very top, above everything -->
  ...
  <!-- Card panel below, with its own wallet tab disabled -->
  <checkout-cc-panel data-applepay="false"></checkout-cc-panel>
  <div data-checkout-message class="checkout-message"></div>
...
The key difference is the standalone `<checkout-wallet />` at the top, with the card panel's own wallet disabled below it. Customers with Apple Pay or Google Pay see the wallet buttons immediately; everyone else sees the card form and fills in shipping as usual.

`checkout.wallet_visible` flips to `true` as soon as a wallet button actually renders, so the divider only appears when there is something above it. `force="true"` pre-renders the native Apple Pay button on iOS and sets `wallet_visible` immediately, avoiding a layout shift while detection runs.

<Info>
Both examples use the same `checkout-details` and `checkout-cc-panel` tags, and both submit through `[data-checkout-submit]`. The backend processes them identically — the only difference is where the blocks appear in the template. (`checkout-button` is a **page builder block name**, not an HTML tag; in the code editor use `<button data-checkout-submit>`.)
</Info>

---

## Example 3: Minimal digital product checkout
{/* SCREENSHOT: Example 3: Minimal digital product checkout · relevant screen or example output · desktop */}

For digital products where you only need email and payment — no name or address fields. The card panel automatically adds a cardholder name field and a Country + ZIP billing block, since neither is on the page.
...
<form id="checkout-form" method="POST">
  ...
  <div data-checkout-message class="checkout-message"></div>
  <button type="button" data-checkout-submit class="checkout-button">Purchase</button>
</form>
```

What happens automatically:

- **Apple Pay** appears as a radio tab alongside Card. On NMI, since only email is on the page, the wallet requests only email — no address sheet. (Stripe always requests name, email and shipping.)
- **Card panel** adds a cardholder name field, since `first_name` / `last_name` aren't on the page.
- **Billing panel** adds a Country select and a ZIP / postal code input, since neither `billing_zip` nor `shipping_zip` is on the page.
- **Billing values** are automatically copied to shipping during submission.

<Note>
To render the wallet buttons **above** the panel instead of as a tab, add `<checkout-wallet force="true" />` above it and set `data-applepay="false"` on the panel, as in Example 2. Setting `data-applepay-layout="separated"` on `<checkout-cc-panel>` does not work — the attribute is dropped during server-side tag expansion.
</Note>

---

## Configuration options
{/* SCREENSHOT: Configuration options · relevant screen or example output · desktop */}
...
| `checkout_settings.billing_fields` | Which fields the card panel's auto-billing block shows — `['country', 'zip']`. Omit for both; `'address'` is a legacy alias for `'zip'`. Exposed as `checkout.panel_billing_fields` |
...
{/* SCREENSHOT: Page builder vs code editor · relevant screen or example output · desktop */}
...
- **Customer Flag** — consent / opt-in checkbox (`data-flag-key`, e.g. `allow_emails`)
- **Single Product Summary** — product image/title for single-product checkout
- **Cart Products** — product list for multi-item checkout

Drag these blocks in your preferred order. For a wallet-first layout, add a `<checkout-wallet />` above **Checkout Details** via a custom code block and set `data-applepay="false"` on the **Credit Card Form**.
