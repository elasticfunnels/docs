...
<Warning>
**`checkout.shipping` renders `$0.00` when the buyer's region has no rate defined.** Shipping comes from the product's shipping profile, and its **Mainland US** and **International** rates are independent - an empty rate is treated as free shipping for that region, not as a fallback to the other rate. A blank International Cost therefore ships free to every non-US buyer, and a product with no shipping profile ships free everywhere. See [How a shipping rate is chosen](/products/product-management).
</Warning>
...
setVariable("checkout_settings", {
  // Optional: restrict countries / US states (see 4.1)
  // Optional: order bumps the page always offers (see 4.2)
...
<Warning>
**`setVariable` is what publishes the settings. A bare `var` does nothing.**

Backend scripts run in a sandbox, and the only value that leaves it is what you hand to
`setVariable`. Writing `var checkout_settings = {...}` and stopping there leaves the object
inside the sandbox where nothing reads it. Nothing errors and the page renders normally, so the
mistake is invisible: your country allowlist is silently not applied and your bumps never appear.

If you would rather build the object up in steps, that is fine. Just publish it at the end:

```html
<script scope="backend">
var checkout_settings = {};
checkout_settings.countries = ['US'];
setVariable("checkout_settings", checkout_settings);
</script>
```
</Warning>
...
### 4.2 Order bumps the page always offers

`checkout_settings.bumps` names the order bumps this checkout page offers, so they do not depend
on the URL or on a funnel run. Each entry is a **bump product code**, or an object carrying
overrides:
...
setVariable("checkout_settings", {
  bumps: [
    { code: 'bump-shipping-protection', description: 'Covers loss in transit', added_message: 'Protected' }
  ]
});
</script>
```
...
1. Each code is resolved against the product catalog. A product whose classification is not
   `bump` is skipped, so a typo shows nothing rather than selling the wrong item.
2. `name`, `price` and `image` always come from the product record. `description`,
   `image` and `added_message` given here override it when non-empty.
3. These merge with any bumps from `?bumps=` and from the **Set Checkout Bumps** node. A code
   declared in more than one place is offered once.

<Tip>
Prefer this over the URL when a page should always carry the same bump. Because it lives on the
page, it survives a direct hit, a refresh and a shared link, none of which is true of `?bumps=`.
Use the **Set Checkout Bumps** node instead when the bump set is decided per funnel branch or
is being split-tested.
</Tip>

### 4.3 Card panel billing fields
...
<script scope="backend">
setVariable("checkout_settings", {
  billing_fields: ['country']   // country only, no ZIP
});
</script>
```
...
`checkout_settings.wallet` is surfaced as `checkout.wallet_settings` and its values are handed
straight to Apple on the **Apple Pay recurring payment sheet**. They are not an ElasticFunnels
concept: Apple defines these fields, and Apple requires them on a recurring transaction.
...
| Key | Goes to | Falls back to |
|-----|---------|---------------|
| `recurring_description` | the sheet's description of what recurs | the product title, else `Subscription` |
| `recurring_label` | the line item label on the sheet | the product title, else `Subscription` |
| `management_url` | the "manage subscription" link Apple shows the customer | the page's own origin |

<Warning>
`management_url` should point at a page where the customer can genuinely view and cancel the
subscription, such as your members area. It is shown to the buyer inside Apple's sheet and in
their Apple Pay subscription settings afterwards. Leaving it unset silently falls back to the
checkout page's origin, which is rarely a page that can actually cancel anything.
</Warning>

These apply only when **all** of the following hold: the gateway is NMI with Collect.js, the
product is a subscription, and the page has an Apple Pay container. On a one-time purchase they
are ignored.
...
- **Billing block:** If neither `billing_zip` nor `shipping_zip` is on the page, the card panel renders a billing block containing a **Country** select and a **ZIP / postal code** input. It never adds a name or street-address field. Which of the two appear is controlled by `checkout_settings.billing_fields` (section 4.3).
