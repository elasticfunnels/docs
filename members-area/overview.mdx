...
## How It Works
{/* SCREENSHOT: How It Works · members area screen · desktop */}
...
## Key Components
{/* SCREENSHOT: Key Components · members area screen · desktop */}
...
{/* SCREENSHOT: Auto-Login After Purchase · members area screen · desktop */}
...
{/* SCREENSHOT: Protecting Pages with Login · members area screen · desktop */}
...
{/* SCREENSHOT: Auth Wrapper Pattern (recommended) · members area screen · desktop */}
...
### "My Library" pattern — `getPurchasedProducts()`
...
<h1>My Library</h1>
@foreach(item in var.library)
  <article class="library-card">
    <img src="{{ item.image }}" alt="{{ item.title }}" />
    <h2>{{ item.title }}</h2>
    <p class="meta">Order #{{ item.order_number }} — {{ item.created_at | date:'dd MMM yyyy' }}</p>

    {{-- Downloads (pre-rolled-up, deduped, kind-tagged) --}}
    @if(item.downloads.length > 0)
      <ul class="downloads">
        @foreach(d in item.downloads)
          <li>
            <a href="{{ d.url }}">
              <span class="kind">{{ d.kind }}</span> {{ d.title | default:'Download' }}
              @if(d.extension)<small>.{{ d.extension }}</small>@endif
            </a>
          </li>
        @endforeach
      </ul>
    ...
    {{-- Included bonuses (attached to THIS order — every is_bonus line) --}}
    @if(item.included_bonuses.length > 0)
      <section class="bonuses">
        <h3>Included Bonuses</h3>
        @foreach(b in item.included_bonuses)
          <div class="bonus">
            <h4>{{ b.title }}</h4>
            @if(b.bonus_url)
              <a href="{{ b.bonus_url }}">Download</a>
            @endif
          </div>
        @endforeach
      </section>
...
## Template Variables
{/* SCREENSHOT: Template Variables · members area screen · desktop */}
