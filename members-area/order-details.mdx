...
ElasticFunnels provides two ways to display order information on your pages: the `<order>` HTML tag (for the visual builder) and backend functions like `getOrders()` / `getOrder()` for code-based pages. Both retrieve data from the customer's purchase history.
...
| `getOrders()` / `getOrder()` | Code-based pages, complex layouts with full template control | Backend scripts / `.ef` pages |
