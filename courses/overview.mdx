...
Progress is tracked server-side per customer, per lesson. The course detail page receives the customer's completed lesson IDs via the `course` object. The front-end marks lessons complete by calling the progress API (`/api/course/progress`).

The `getCourseForCustomer(slug)` response includes:

- `course.progress_percent` — 0–100
- `course.completed_content_ids` — array of completed lesson IDs
...
```javascript
{
  ...
  // Only on list calls (getCourses, getCoursesForCustomer):
