...
Select a product from the active campaign. Once selected, it's added to the order lines table automatically — there is no separate "Add" button.
...
A sidebar panel shows the campaign's configured sales scripts (main pitch, rebuttal, upsell talking points) for the agent to reference during the call. Scripts are loaded based on the selected call center and brand.
