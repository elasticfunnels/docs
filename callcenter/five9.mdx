...
<Tip>
Complete the general phone switch flow first: **Call Center** → **Call Centers** → open the center → **Brand Settings** → **Phone Switch** → select Five9, then save.
</Tip>
...
<Steps>
  ...
  <Step title="Align agent extensions">
    Ensure each agent’s extension in **Call Center** → **Agents** matches the identifier your API uses for `agent_extension` on make-call.
  </Step>
