...
## Where to configure
{/* SCREENSHOT: Where to configure · merchant setup screen or postback state · desktop */}
...
3. For each event you care about, set:
   ...
   - **Authorization** - a token we send as `Authorization: Bearer <token>`, so you can gate the endpoint at a reverse proxy before it ever reaches your app. Use **Generate token** for a random one, or paste your own. Leave it blank and we generate one for you when you save - the field is never actually empty, so expect the header on every request.
...
## Supported events
{/* SCREENSHOT: Supported events · merchant setup screen or postback state · desktop */}
...
## Request format
{/* SCREENSHOT: Request format · merchant setup screen or postback state · desktop */}
...
X-EF-Brand-Id: 42
X-EF-Merchant-Id: 17
Authorization: Bearer <the token on that URL's row>
```
...
| `Authorization`       | Bearer token for that URL's row. Snapshotted when the delivery is queued, so retries keep sending the token that was in effect at the time - rotating it will not change an already-queued delivery. |
...
{/* SCREENSHOT: Standardized payload · merchant setup screen or postback state · desktop */}
...
Six fields in `tracking` are yours to populate - `vtid` and `subid` through `subid5`. Put any of them on the landing URL or on the buy link as a query parameter and we capture, store, and echo the value back on every postback for that order:

```
https://yourdomain.com/lander?vtid=YOUR_KEY
https://yourdomain.com/b?p=product_1&vtid=YOUR_KEY
```

- **`vtid`** - "vendor tracking ID". A single general-purpose slot, independent of gateway and of any affiliate network. Best choice for a third-party tracker or session key that you need reflected back. Keep values under **512 characters**: the postback echoes the value in full, but the click record truncates at 512, so a longer value will not match between the two when you reconcile. This matters most for base64/encoded composite keys, which get long quickly.
- **`subid` .. `subid5`** - five slots conventionally used for affiliate-network sub-tracking (RedTrack, Voluum, and similar).

All six are also filterable and exportable from the **Conversions** report, so you can reconcile against the postbacks you received.

<Note>
Upsell and rebill conversions inherit these values from the original purchase, so a subscription's renewals carry the same `vtid` as the sale that created it.
</Note>
...
{/* SCREENSHOT: Example: minimal Express.js handler · merchant setup screen or postback state · desktop */}
...
## Operational details
{/* SCREENSHOT: Operational details · merchant setup screen or postback state · desktop */}
