...
Update a component's **metadata only** — name, code slug, and html_only flag. This endpoint does **not** accept or persist `html`, `css`, or `config`. To update a component's content, delete it and re-create it with [Create Component](#create-component).

<Warning>
  **`html`, `css`, and `config` are silently ignored on PUT.** The endpoint returns `200` even if you include them — but the content is not saved. This is by design: visual-builder component content is managed internally by the editor and not writable through the public API.
</Warning>
...
<Warning>
  **Common causes of errors on PUT:**
  ...
  | `html`/`css`/`config` not updating | These fields are not writable via PUT | Use DELETE + POST to replace a component's content |
</Warning>
