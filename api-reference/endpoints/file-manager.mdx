...
<Warning>
  **`POST /api/brands/{brand}/media` does not exist** (returns 404). Use [Upload File](#upload-file) (`/file-manager/upload-file`) or [Legacy upload](#legacy-upload-brand-files) (`/files/upload`) below.
</Warning>
...
```
POST /api/brands/{brand}/files
```

```bash cURL
curl -X POST https://app.elasticfunnels.io/api/brands/{brand_id}/files \
  -H "EF-Access-Key: your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```json Response
{
  "data": [
    {
      "src": "https://cdn.elasticfunnels.io/42/assets/media/logo.png?t=1733846400",
      "type": "image",
      "width": 200,
      "height": 80
    }
...
```
POST /api/brands/{brand}/files/upload
```
...
```bash cURL
curl -X POST https://app.elasticfunnels.io/api/brands/{brand_id}/files/upload \
  -H "EF-Access-Key: your_api_key_here" \
...
```json Response
{
  "data": [
    {
      "src": "https://cdn.elasticfunnels.io/42/assets/media/logo.png",
      "type": "image",
      "width": 400,
      "height": 120
    }
...
<Note>
  Prefer [Upload File](#upload-file) (`/file-manager/upload-file`) for new integrations — it supports paths, custom filenames, and more MIME types.
</Note>
