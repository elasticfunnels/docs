...
## Translations
{/* SCREENSHOT: Translations · backend script editor with t() · desktop */}

Backend scripts use the same **resolved locale** as page templates (`req.locale` after host resolution). That value comes from route/query/session/domain/brand/`Accept-Language` and is then **snapped** to a row in Advanced → Translations when the brand has languages configured. See [Internationalization](/pages/internationalization) for precedence and template helpers (`t()`, `| t`, and related).

Most translation helpers are **async** in the sandbox — use `await`.
...
```javascript
if ((await getLocale()) === 'ro') {
  setVariable('currency_symbol', 'lei');
}
```
...
```javascript
var available = await getLocales(); // ['en', 'ro']
```
...
```javascript
if (await getDefaultLocale() === 'en') {
  setVariable('show_usd', true);
}
```
...
setVariable('checkout_cta', cta);
var pct = await t('product.savings_percent', { percent: 20 });
setVariable('savings_label', pct);
```
...
if (code === 'de') {
  setVariable('region', 'EU');
}
```

## Orders
{/* SCREENSHOT: Orders · code editor or rendered example · desktop */}
...
## Bonus Products
{/* SCREENSHOT: Bonus Products · code editor or rendered example · desktop */}
...
## Subscriptions
{/* SCREENSHOT: Subscriptions · code editor or rendered example · desktop */}
...
## Conversions
{/* SCREENSHOT: Conversions · code editor or rendered example · desktop */}
...
## Products
{/* SCREENSHOT: Products · code editor or rendered example · desktop */}
...
### `resolveVariant(product, code?)`

Pick which variant a page should start on. Returns `null` for a product with no
variants, so you can branch on it.

```javascript
var product = getProduct('menosoothe');
var picked  = resolveVariant(product, request.route_params.code);

if (picked) {
  setVariable('price', picked.price);
  setVariable('buy_code', picked.code);   // what add-to-cart submits
}
```

`code` may be a **variant code** or the **linked product's code**, so a URL like
`/shop/product/menosoothe-3` resolves without you mapping it yourself. With no match,
or no code, the product's default variant is returned.

### Variants on a product

Every product returned by `getProduct`/`getProducts` carries a normalised `variants`
array, ordered by position, plus `product_fields` (the option axes) and `has_variants`.

```javascript
var product = getProduct('menosoothe');

@foreach(v in product.variants)
  {{ v.title }}          // "3 Bottles"  - joined option values
  {{ v.code }}           // "menosoothe-3" - resolved through the linked product
  {{ v.price }}          // 177
  {{ v.retail_price }}   // 297
  {{ v.per_unit_price }} // 59  - price / units, precomputed
  {{ v.on_sale }}        // true when retail_price > price
  {{ v.is_default }}     // exactly one variant is true
@endforeach
```

| Field | Notes |
|---|---|
| `code` | Variant code, else the linked product's code, else the variant id. This is what you submit to the cart. |
| `linked_product_id` / `linked_product_code` | The product this variant resolves to. `null` for a standalone variant. |
| `title` | Option values joined with ` / `, or `Default` when the product has no option axes. |
| `option_values` | Object keyed by option name, e.g. `{ "Pack size": "3 Bottles" }`. |
| `per_unit_price` | `price / units`, or `null` when `units` is unset. |
| `on_sale` | `true` when `retail_price` is greater than `price`. |
| `position` | Display order. The array is already sorted by it. |
| `is_default` | Exactly one variant per product is `true`. |

<Note>
Commercial fields resolve through the linked product (see
[Product Variants](/products/variants)), so `price`, `sku` and `image` are already the
values you should show — no second lookup needed.
</Note>
...
The same functions are available in **backend** page templates under the same names (`getProduct` / `getProducts` / `getAllProducts`), see [Products, categories & shipping (backend)](/backend-template-engine/products-and-shipping).
...
The same helper is available in backend page templates under the same name, see [Products, categories & shipping (backend)](/backend-template-engine/products-and-shipping).
...
## Courses
{/* SCREENSHOT: Courses · code editor or rendered example · desktop */}
...
## Blog
{/* SCREENSHOT: Blog · code editor or rendered example · desktop */}
...
## Purchase Links
{/* SCREENSHOT: Purchase Links · code editor or rendered example · desktop */}
...
## Assets
{/* SCREENSHOT: Assets · code editor or rendered example · desktop */}
...
## Visitor Control
{/* SCREENSHOT: Visitor Control · code editor or rendered example · desktop */}
...
## Session Items
{/* SCREENSHOT: Session Items · code editor or rendered example · desktop */}
...
## CRM Data
{/* SCREENSHOT: CRM Data · code editor or rendered example · desktop */}
