...
Data functions that load or save brand data are **asynchronous** under the hood, but they look synchronous in your code — just call them and use the return value directly. Pure-computation helpers (like `formatPrice`, `productSavings`) are truly synchronous.
...
Fetch **one** order for the current customer. Same scope as `getOrders`. The `code` argument is required: public order ID (`order_number`), numeric order id, or conversion `code`. Returns `null` if not found.
...
<Tip>
These APIs return **filtered** data only. Raw provider API responses and full fulfillment payloads are not available in backend scripts.
</Tip>
...
CRM functions read and write **entries** for the current brand. Every call is scoped by:

- **Brand** — you never pass a brand id; the current brand is applied automatically.
...
- **`referenceType` + `referenceId`** — which record the entry is attached to. Defaults: `referenceType` → `'customer'`, `referenceId` → current session **`customer_id`** when omitted or `null`.
...
**`async`** (optional boolean, default `false`) — on **`setCrmField`** and **`reassignCrmReference`**, when `true` the function returns as soon as the change has been accepted; the update may complete a moment later. Use the default when you need the value to be readable immediately on the same request or the next page. Write failures do not throw back into your script. **`getCrmField`** / **`getCrmEntries`** accept `async` for a consistent options shape; it has no effect on reads. Alias: **`crm_async`**.
...
### `GetCrmEntries({ slug, referenceType?, referenceId?, limit?, async? })`
...
| `async` | boolean | `false` | No effect on reads |
...
Entries may include additional fields (for example pipeline or stage metadata) depending on your entity configuration.

### `GetCrmField({ slug, fieldKey, referenceType?, referenceId?, async? })`
...
### `SetCrmField({ slug, fieldKey, value, referenceType?, referenceId?, async? })`
...
| `referenceType` | string | `'customer'` | Linked record type |
| `referenceId` | string | session `customer_id` | Linked record id |
| `async` | boolean | `false` | When `true`, returns `true` once the write is accepted (may finish shortly after) |
...
// Fire-and-forget when you don’t need the next page to read the value immediately
setCrmField({ slug: 'pattern-tracker', fieldKey: 'draft_blob', value: payload, async: true });
```

### `ReassignCrmReference({ slug, fromReferenceType, fromReferenceId, toReferenceType?, toReferenceId?, async? })`

Moves **all** CRM entries for one **reference** (same entity `slug`) to another reference. Use this when anonymous funnel data was stored under e.g. `quiz_visitor` + a session UUID and should attach to the logged-in **`customer`** + `customer_id` after members login.

Returns an object:

```javascript
{ ok: true, migrated: 2, merged: false }  // entries updated in place
{ ok: true, migrated: 1, merged: true }  // merged into existing customer row(s), visitor docs removed
{ ok: true, pending: true, migrated: 0, merged: false }  // async: true — reassignment still in progress
{ ok: false, error: 'forbidden', migrated: 0, merged: false }
```

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `slug` | string | **required** | CRM entity slug |
| `fromReferenceType` | string | **required** | Source `reference_type` (e.g. `quiz_visitor`) |
| `fromReferenceId` | string | **required** | Source `reference_id` |
| `toReferenceType` | string | `'customer'` | Target type (optional; defaults to `customer`) |
| `toReferenceId` | string | session `customer_id` | Target id (optional; defaults to session `customer_id`) |
| `async` | boolean | `false` | When `true`, returns immediately with `pending: true` while reassignment continues |

**Security:**

- `is_customer` must be true and session `customer_id` must be set.
- `fromReferenceId` must **exactly match** `session.get('quiz_visitor_id')` so visitors cannot reassign arbitrary CRM rows.
- The effective target is always the current session’s customer: if you pass `toReferenceType` / `toReferenceId`, they must still resolve to **`customer`** + that same `customer_id` (stray values return `forbidden_target`).

**Merge behavior:** If the **target** reference already has CRM entries, field values are merged with **customer fields winning** on duplicate keys; duplicate entry records are consolidated. Calling again when the source reference has no rows returns `{ ok: true, migrated: 0, merged: false }` (idempotent).

```javascript
// After login, session has quiz_visitor_id + customer; target defaults to customer
var r = reassignCrmReference({
  slug: 'pattern-tracker',
  fromReferenceType: 'quiz_visitor',
  fromReferenceId: session.get('quiz_visitor_id'),
});
if (r.ok && r.migrated > 0) {
  session.set('quiz_visitor_id', null);
}
```

<Note>
`slug` must match a CRM entity configured for your brand in CRM settings.
</Note>
