...
Catalog products can include attached downloads. Products from **`getProduct`**, **`getAllProducts`**, **`getProductByCode`**, and **`getProducts()`** (unfiltered list path) are loaded through the same cache layer as templates and include the **normalized** fields below. If you call **`getProducts`** with **type**, **classification**, or **codes** filters, you still get **`product_files`** plus primary **`bonus_file`** and **`digital_file`** from the database; iterate **`product_files`** and check **`purpose`** (`digital` / `bonus`) when you need consistent behavior in every case.

Use these fields:

| Field | Type | Description |
|-------|------|-------------|
| **`product_files`** | array | All file rows. Each item includes **`purpose`** (`digital` or `bonus`), **`title`**, **`description`**, **`image`**, **`file`** (URL), **`sort_order`**, **`is_primary`**, and optional **`metadata`**. |
| **`bonus_file`** | string | URL of the **primary** bonus-purpose file, when one is set. |
| **`digital_file`** | string | URL of the **primary** digital-purpose file, when one is set. |
| **`bonus_files`** / **`digital_files`** | array | **Purpose-filtered** lists cut from `product_files` (same object shape; convenient for looping one type). |
| **`primary_download_url`** | string | First primary downloadable file in sort order, any purpose. |

**Example** — first bonus file for a product:

```javascript
var p = getProduct('my-sku');
if (p && p.bonus_file) {
  setVariable('bonus_href', p.bonus_file);
}

if (p && p.product_files && p.product_files.length) {
  for (var i = 0; i < p.product_files.length; i++) {
    var row = p.product_files[i];
    if (row.purpose === 'digital' && row.file) {
      console.log(row.title, row.file);
    }
  }
}
```
