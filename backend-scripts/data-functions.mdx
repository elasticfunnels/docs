...
| `metadata` | object | Optional metadata object. Always an object (parsed from JSON when stored that way). |

**Example** — pick the primary bonus URL with a safe fallback, then list all digital files:
...
if (p && p.digital_files && p.digital_files.length) {
  for (var i = 0; i < p.digital_files.length; i++) {
    ...
    console.log(row.title, row.file, 'primary?', row.is_primary);
  }
}
```
