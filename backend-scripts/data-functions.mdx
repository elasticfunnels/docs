...
CRM functions let you read and write structured data stored in CRM entries. Entries are linked to any record via `reference_type` and `reference_id` — most commonly customers.

Pass `null` for the first two parameters to use the defaults — this is the most common pattern:

- **`referenceType`** = `null` → defaults to `'customer'`
- **`referenceId`** = `null` → defaults to the current session's `customer_id`

So `setCrmField(null, null, 'score', 85)` stores `score = 85` on the current customer.

### `GetCrmEntries(referenceType?, referenceId?, limit?)`

Fetch CRM entries linked to a reference record. Defaults to the current customer if no reference is given.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `referenceType` | string | `'customer'` | Type of the linked record |
| `referenceId` | string | session customer_id | ID of the linked record |
| `limit` | number | `50` | Max entries to return (max 200) |

```javascript
// Get all CRM entries for the current customer (null = use session customer)
var entries = getCrmEntries(null, null, 50);

for (var i = 0; i < entries.length; i++) {
  console.log(entries[i].title, entries[i].values_flat);
}

// Get entries for a specific reference type and ID
var orderEntries = getCrmEntries('order', '12345');
```
...
### `GetCrmField(referenceType?, referenceId?, fieldKey)`

Get a single field value from the first matching CRM entry. Searches across all entries for the reference.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `referenceType` | string | `'customer'` | Type of the linked record |
| `referenceId` | string | session customer_id | ID of the linked record |
| `fieldKey` | string | **required** | The field key to look up |

```javascript
// Get a field for the current customer
// null, null = use 'customer' type + session customer_id
...
// Explicit reference type — still uses session customer_id
var goal = getCrmField('customer', null, 'goal_weight');
```

### `SetCrmField(referenceType?, referenceId?, fieldKey, value)`

Set a field value on a CRM entry. If the field exists on an entry, it is updated in place. If no entry has that field, a new entry is created.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `referenceType` | string | `'customer'` | Type of the linked record |
| `referenceId` | string | session customer_id | ID of the linked record |
| `fieldKey` | string | **required** | The field key to set |
| `value` | any | **required** | The value (string, number, boolean, or object) |
...
// Set a field on a different reference type (explicit ID)
setCrmField('order', '12345', 'followup_status', 'contacted');
```

<Note>
`SetCrmField` creates a generic CRM entry (with `crm_entity_id: 0`) if no matching entry exists. For structured data with pipelines and stages, create entries through the CRM management UI and use `SetCrmField` to update individual fields.
</Note>
