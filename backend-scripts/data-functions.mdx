...
For full request/response shapes and ready-to-use page-based examples, see [Subscriptions (backend template engine)](/backend-template-engine/subscriptions).
...
Fetch a single subscription by its **original transaction ID** (`original_transaction_id` on the subscription record). Returns `null` if not found or if the subscription does not belong to the current customer.
...
### `getCourseBySlug(slugOrOpts)`

Fetch a course by its URL slug.

```javascript
// Preferred object style
var course = getCourseBySlug({ slug: 'javascript-basics' });

// Legacy positional style still supported
var course = getCourseBySlug('javascript-basics');
```

### `getModuleBySlug(courseSlugOrOpts, moduleSlug?)`

Fetch a single module with its contents.

```javascript
// Preferred object style
var module = getModuleBySlug({ course: 'javascript-basics', module: 'variables' });
...
```javascript
// Preferred object style
var courses = getCourses({ assignedOnly: false, sort: 'enrollment_oldest_unfinished_first' });

// Legacy positional style still supported
var coursesLegacy = getCourses();
setVariable('course_count', courses.length);
```

### `getCoursesByCategorySlug(categoryOrOpts, assignedOnly?)`

Filter courses by category slug.

```javascript
// Preferred object style
var marketingCourses = getCoursesByCategorySlug({
  category: 'marketing',
  assignedOnly: false,
...
// Legacy positional style still supported
var marketingCoursesLegacy = getCoursesByCategorySlug('marketing');
...
```javascript
var categories = getCategoriesWithCourses({ assignedOnly: false });
var categoriesLegacy = getCategoriesWithCourses();
// [
...
### `getCourseForCustomer(slugOrOpts)`
...
```javascript
var course = getCourseForCustomer({ slug: 'javascript-basics' });
var courseLegacy = getCourseForCustomer('javascript-basics');
...
Fetch all courses the current customer is enrolled in. Same as `getCourses({ assignedOnly: true })`.

```javascript
var myCourses = getCoursesForCustomer({ sort: 'enrollment_oldest_unfinished_first' });
var myCoursesLegacy = getCoursesForCustomer();
```

Each course includes `module_count`, `lesson_count`, `progress_percent`, `completed_content_ids`, and `is_assigned`.

### `getCourseLesson(courseOrOpts, moduleId?, lessonId?)`

Resolve a lesson-player payload in one call (course + modules + current lesson + prev/next URLs + progress stats).

```javascript
// Preferred object style
var seed = getCourseLesson({ course: 'javascript-basics', module: 12, lesson: 77 });

// Legacy positional style still supported
var seedLegacy = getCourseLesson('javascript-basics', 12, 77);
```

### `getCoursesForCustomerByCategorySlug(categoryOrOpts)`
...
```javascript
var marketing = getCoursesForCustomerByCategorySlug({ category: 'marketing' });
var marketingLegacy = getCoursesForCustomerByCategorySlug('marketing');
setVariable('marketing_courses', marketing);
```

### `getCategoriesWithCoursesForCustomer(opts?)`
...
```javascript
var categories = getCategoriesWithCoursesForCustomer({ sort: 'enrollment_oldest_unfinished_first' });
var categoriesLegacy = getCategoriesWithCoursesForCustomer();
// [
//   { category_slug: 'marketing', category_name: 'Marketing', courses: [...] },
//   { category_slug: 'design',    category_name: 'Design',    courses: [...] }
// ]
...
var courseDoneLegacy = getCompletedLessonsCount('javascript-basics');
```
...
var singleLegacy = getCourseProgress('javascript-basics');
```
