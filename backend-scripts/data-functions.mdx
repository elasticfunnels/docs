...
## Translations
{/* SCREENSHOT: Translations · backend script editor with t() and getLocale() · desktop */}

Backend scripts can read the same per-brand translation entries that power `data-ef-hl` bindings and the template `t()` helper. The active locale is resolved per request from the URL (`hl`/`locale`/`lang`), session, brand defaults, and `Accept-Language` — backend scripts do not pick a locale themselves.

See [Internationalization](/pages/internationalization) for the schema and admin UI.

### `t(key, fallback?)`

Returns the translation for `key` in the active locale, falling back to the brand's default language entry, then to `fallback`, then to an empty string.

```javascript
var label = t('hero.title', 'Welcome');
setVariable('hero_title', label);
```

### `getLocale()`

Returns the active locale code resolved for this request (e.g. `'en'`, `'pt-br'`). Empty string when the brand has no languages configured.

```javascript
if (getLocale() === 'pt-br') {
  setVariable('currency_symbol', 'R$');
}
```

### `getLocales()`

Returns the list of locale codes configured for the brand, in admin sort order. Useful for rendering a language switcher.

```javascript
var available = getLocales(); // ['en', 'pt-br', 'de']
```

### `getDefaultLocale()`

Returns the brand's default locale code, or empty string if no language is flagged as default.

```javascript
if (getLocale() !== getDefaultLocale()) {
  setVariable('show_default_link', true);
}
```
...
For the same field names in **backend** page templates (`GetProduct` / `GetProducts` / `GetAllProducts`), see [Products, categories & shipping (backend)](/backend-template-engine/products-and-shipping).
...
The same helper is available in backend page templates as `ProductDownloads(product)` — see [Products, categories & shipping (backend)](/backend-template-engine/products-and-shipping).
