...
5. Add a "Component Split Test" node if needed, ensuring you've selected a Split Test Container
...
2. Add a "Split Test Container" on the page and fill in its **Split Test Container Name** trait
3. Go to the page events builder:
   - Click "Add Node" and select **"Component Split Test"**
   - Select the container and set **Total Variants** — a **Traffic Distribution** node is created for each variant
   - Set the traffic percentage on each Traffic Distribution node, then attach a **Component** node under it and pick the component to show
   - Mark one component as the **baseline** (the control)
4. Ensure connections:
   - Connect the Component Split Test node to the "entry node" or a condition node if not already connected

Full walkthrough: [Split Testing](/funnels/split-testing).
...
5. Click "Add Node" and select the **"On Exit Intent"** node — it must be connected to the entry node or a Script Rule
6. Add a **"Show Popup"** node, connect it to the On Exit Intent node, and select your popup
...
4. Add a "Dynamic Content" node to ensure content replacement

**Manual Setup:**
1. **Save your variations as components:** build each version and use **Transform to Component**. Raw divs and sections cannot be swapped.
2. **Wrap the slot:** in the page builder, right-click the element (e.g. packages) and choose **Wrap in → Dynamic Container**. Fill in its **Dynamic Container Name** trait so you can find it later.
3. **Define the condition:** in the page events builder, add a condition node — **Referred By Affiliate** or **Is From Country** where they fit, otherwise **Script Rule**.
4. **Set up the swap:** off the condition's **Output 1** (the true branch), add a **Dynamic Content** node and pick the container and the component to render.

Full walkthrough: [Dynamic Content](/funnels/dynamic-content).
