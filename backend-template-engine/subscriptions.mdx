...
description: 'Backend template functions for listing and managing customer subscriptions — with ready-to-use page-based API examples.'
---
...
| **`getSubscription(transactionId)`** | object \| null | Single subscription by original transaction ID. Returns `null` if not found or not owned by the customer. |
