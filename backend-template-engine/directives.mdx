---
title: 'Directives'
description: 'Backend template directives: @if, @else, @foreach, @set, session storage, @component, @extends, @block.'
---
...
{/* SCREENSHOT: Conditionals: `@if`, `@else`, `@elseif`, `@endif` · code editor or rendered example · desktop */}
...
{/* SCREENSHOT: Loops: `@foreach` … `@endforeach` · code editor or rendered example · desktop */}
...
## Variables: `@set`
{/* SCREENSHOT: Variables: `@set` · code editor or rendered example · desktop */}
...
{/* SCREENSHOT: Components: `@component` · code editor or rendered example · desktop */}
...
{/* SCREENSHOT: Inheritance: `@extends`, `@block`, and `@yield` · code editor or rendered example · desktop */}
...
| | `@block("name") default @endblock` | `@yield("name")` |
|-|---|---|
| Default content | Yes — rendered when child doesn't override | No — renders empty |
| Typical use | Optional sections (header, sidebar, scripts) | Required slots (title, main content, page-level style) |

Example base layout using `@yield` for required slots and `@block` for optional ones:
...
<body>
  @block("header")<header><a href="/">Home</a></header>@endblock
...
@block("style").about { padding: 80px 0; }@endblock
...
## Session storage
{/* SCREENSHOT: Session storage · code editor or rendered example · desktop */}
