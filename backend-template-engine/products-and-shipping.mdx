...
- **`slug`** — SEO-friendly URL slug. Auto-generated from checkout_title/title if not manually set in the SEO tab. Used with [wildcard routes](/pages/wildcard-routes) for clean product URLs (e.g. `{{ product | productUrl:'shop/product' }}` → `/shop/product/my-product-123`).
...
| **`productUrl`** filter | SEO-friendly product URL: `{{ product \| productUrl:'shop/product' }}` → `/shop/product/{slug}-{id}`. Use with [wildcard routes](/pages/wildcard-routes) for product detail pages. |
