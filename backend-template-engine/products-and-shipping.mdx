...
- **`product_files`** — downloadable files attached to the product. Each row includes fields such as **`purpose`** (`digital` or `bonus`), **`title`**, **`description`**, **`image`**, **`file`**, **`is_primary`**, **`sort_order`**, and **`metadata`**.
