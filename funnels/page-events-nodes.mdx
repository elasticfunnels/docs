...
Checks for a tag set by **Tag User**, `ef.addTag()`, or [`addTag()` in a backend script](/backend-scripts/tags).
...
<Warning>
A backend script can read and write the same tags with [`hasTag()` / `addTag()` / `removeTag()`](/backend-scripts/tags), which handle the naming split below for you.
