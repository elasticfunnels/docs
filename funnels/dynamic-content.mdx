...
The `<dynamic-container>` tag marks a slot on the page whose content can be swapped at render time by a Page Events **Dynamic Content** node.
...
When a matching Page Events Dynamic Content node fires, the server replaces the `<dynamic-container>` with a `<component>` embed that injects the target Custom Component:
...
<dynamic-container id="headline-slot">
  <h1>Default Headline</h1>
</dynamic-container>

<!-- Slot 2: controls the footer disclaimer area -->
<dynamic-container id="footer-disclaimer-slot">
