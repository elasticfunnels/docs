...
{/* SCREENSHOT: When to Use Script Rule · funnel builder, page events, or published flow · desktop */}
...
{/* SCREENSHOT: How Script Rules Work · funnel builder, page events, or published flow · desktop */}
...
3. **Flow Control**: 
   - `true` → **Output 1**, the Yes path
   - `false` → **Output 2**, the No path
...
{/* SCREENSHOT: Available Data Variables · funnel builder, page events, or published flow · desktop */}
...
**Page templates:** The same device and request fields (`is_mobile`, `is_tablet`, `device`, `query`, `is_customer`, etc.) are available in page content. Use `@if(request.is_mobile)` or `@if(is_mobile)` and `{{ request.* }}` — see [Page Variables → Request & device context](/pages/page-variables#request--device-context).
...
return ip_whois && (ip_whois.country_code == 'US' || ip_whois.country_code == 'CA');

// Block certain regions
return ip_whois && ip_whois.country_code != 'CN' && ip_whois.country_code != 'RU';

// Block VPNs, proxies and datacentre traffic
const sec = ip_whois && ip_whois.security;
return !!sec && sec.proxy != true && sec.vpn != true && sec.hosting != true;

// EU visitors
return ip_whois && ip_whois.is_eu == true;

// Timezone-based routing
return ip_whois && ip_whois.timezone.id == 'America/New_York';

// ISP-based routing
const isp = (ip_whois && ip_whois.connection && ip_whois.connection.isp) || '';
return isp.includes('Comcast') || isp.includes('Verizon');

// Block a known network by AS number
return !(ip_whois && ip_whois.connection && ip_whois.connection.asn == 32934);
```
...
{/* SCREENSHOT: Debugging Script Rules · funnel builder, page events, or published flow · desktop */}

### Using the Debug Window

The best way to debug Script Rules is with the **Debug Window**. It shows what happens when your script runs.
...
The Debug Window shows Script Rule details:
...
console.log('Is customer:', is_customer);
console.log('Visit count:', visit_data?.visit_count);
console.log('Country:', ip_whois?.country_code);

// Log your condition results
const isEligible = ip_whois.country_code == 'US' && visit_data.visit_count > 1;
...
## Example Use Cases
{/* SCREENSHOT: Example Use Cases · funnel builder, page events, or published flow · desktop */}

### Example 1: Geographic Targeting

```javascript
// US and Canada only, block VPNs
return (ip_whois.country_code == 'US' || ip_whois.country_code == 'CA') &&
       ip_whois.security.proxy != true &&
       ip_whois.security.hosting != true;
```
...
2. **Break Complex Logic into Steps**
   ...
   const isEligibleCountry = ip_whois.country_code == 'US';
...
1. **Check for Null/Undefined**
   ...
   return ip_whois && ip_whois.country_code == 'US';
...
2. **Provide Fallback Values**
   ...
   const country = ip_whois?.country_code || 'unknown';
   return country == 'US';
   ```
...
if (!ip_whois || !ip_whois.country_code) {
...
{/* SCREENSHOT: JavaScript Limitations · funnel builder, page events, or published flow · desktop */}
...
| Limit | Value |
|-------|-------|
| Execution time | 5 seconds — the script is interrupted past that |
| Memory | 8 MB |
| Stack | 512 KB |
| `eval()` / `new Function()` | Disabled |

A script that throws is treated as **false**, so the flow takes Output 2. The error and any `console` output are captured and shown in the [Debug Window](/debugging/debug-window).
...
- [Page Events Node Reference](/funnels/page-events-nodes) - Every node type and its fields
