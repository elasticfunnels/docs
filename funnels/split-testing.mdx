...
That block is the page source. It is **not** replaced in the API response with rendered output.

At runtime, when a visitor loads the page, the server renders only the chosen variant and replaces the entire block with:
...
1. **On editor save**, the system validates the tag structure (minimum 2 variants, each with a name). It creates a split test database record and injects an `id` attribute into the tag. The editor updates automatically to show the assigned ID.
