...
{/* SCREENSHOT: Where you can use it · funnel builder, page events, or published flow · desktop */}
...
## Tags
{/* SCREENSHOT: Tags · funnel builder, page events, or published flow · desktop */}
...
## Flow control
{/* SCREENSHOT: Flow control · funnel builder, page events, or published flow · desktop */}
...
## Visibility
{/* SCREENSHOT: Visibility · funnel builder, page events, or published flow · desktop */}
...
## Timing
{/* SCREENSHOT: Timing · funnel builder, page events, or published flow · desktop */}
...
## Video
{/* SCREENSHOT: Video · funnel builder, page events, or published flow · desktop */}
...
Runs your **callback** when the video CTA is shown (when `video-cta-shown` fires, e.g. from `getStarted()`). The callback receives the event detail (e.g. `{ focus_on_cta, timestamp }`). Returns an **unsubscribe** function.

```javascript
ef.onCta(function(detail) {
  ef.nextNode();
});
var unsubscribe = ef.onCta(function() { /* ... */ });
// unsubscribe();
```

### ef.getVideoProgress()

Returns **`{ currentTime, duration, percent }`** for the main video. Same sources as the funnel video nodes.
...
{/* SCREENSHOT: Client script examples · funnel builder, page events, or published flow · desktop */}
...
{/* SCREENSHOT: Adding a tag from a click (no script) · funnel builder, page events, or published flow · desktop */}
...
## Summary
{/* SCREENSHOT: Summary · funnel builder, page events, or published flow · desktop */}
...
| `ef.sub.upgrade(old, new)` | Upgrade subscription from one SKU to another. Returns Promise. See [Subscription Upsells](/products/subscription-upsells). |
| `ef.sub.cancel(reason)` | Cancel the current subscription. `reason` is optional and is recorded against the cancellation - pass the exit-survey answer so it reaches your churn reporting. Returns Promise. See [Subscription Upsells](/products/subscription-upsells). |
| `ef.sub.upgradeThenNavigate(old, new, url)` | Upgrade subscription then navigate, with loading overlay. |
| `ef.sub.cancelThenNavigate(url, reason)` | Cancel subscription then navigate, with loading overlay. `reason` is optional; without it the cancellation is recorded as "Not specified". |