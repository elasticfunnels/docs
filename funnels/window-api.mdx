...
| `ef.sub.cancel()` | Cancel the current subscription. Returns Promise. See [Subscription Upsells](/products/subscription-upsells). |
| `ef.sub.upgradeThenNavigate(old, new, url)` | Upgrade subscription then navigate, with loading overlay. |
| `ef.sub.cancelThenNavigate(url)` | Cancel subscription then navigate, with loading overlay. |