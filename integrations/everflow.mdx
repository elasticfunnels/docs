...
4. **Customer makes purchase** → Via BuyGoods, ClickBank, or other payment gateway
...
Unlike BuyGoods or ClickBank integrations (which **receive** conversions), Everflow is **outbound only** - we send conversion data to Everflow for attribution, similar to how Google Ads and Meta Ads integrations work.
...
- Works with BuyGoods, ClickBank, Stripe, PayPal, etc.
...
- **Works with any merchant** - Compatible with BuyGoods, ClickBank, Stripe, or any other payment processor
...
<AccordionGroup>
  ...
  <Accordion title="Can I use Everflow with BuyGoods/ClickBank?">
    **Answer:** Yes! Everflow tracks attribution (which lander generated the conversion), while BuyGoods/ClickBank handles the actual payment processing. They work together perfectly:
    
    1. Customer clicks Everflow link (transaction_id captured)
    2. Customer lands on your page
    3. Customer purchases via BuyGoods/ClickBank
...
The Everflow integration works regardless of which payment gateway you use:
...
Everflow tracks attribution (which link/lander generated the conversion), independent of how payment is processed.
...
<CardGroup cols={2}>
  <Card
    title="Set Up BuyGoods"
    icon="link"
    href="/integrations/buygoods"
  >
    Configure BuyGoods for payment processing
  </Card>
  <Card
    title="Set Up ClickBank"
    icon="link"
    href="/integrations/clickbank"
  >
    Configure ClickBank for affiliate tracking
  </Card>
  <Card
