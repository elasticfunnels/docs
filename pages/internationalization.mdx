...
1. **Wildcard / route params** — `hl` or `locale` captured by a
   [wildcard route](/pages/wildcard-routes) (e.g. the slug `docs/{hl}`), also
   merged Express `params` when present. Note that a slug cannot *start* with a
   placeholder, so `/en/about` cannot be matched this way — use `?hl=en`.
...
- [Wildcard routes](/pages/wildcard-routes) — capture an `hl` path segment
  (e.g. the slug `docs/{hl}`); the placeholder cannot be the first segment.
