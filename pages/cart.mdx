...
{/* SCREENSHOT: Adding a cart to your page · page builder, settings panel, or published page · desktop */}
...
## What visitors see
{/* SCREENSHOT: What visitors see · page builder, settings panel, or published page · desktop */}
...
{/* SCREENSHOT: Customizing the cart layout · page builder, settings panel, or published page · desktop */}
...
## Quantity controls
{/* SCREENSHOT: Quantity controls · page builder, settings panel, or published page · desktop */}
...
## Cart dropdown
{/* SCREENSHOT: Cart dropdown · page builder, settings panel, or published page · desktop */}
...
## Checkout link
{/* SCREENSHOT: Checkout link · page builder, settings panel, or published page · desktop */}
...
## Cart storage
{/* SCREENSHOT: Cart storage · page builder, settings panel, or published page · desktop */}
...
## JavaScript API
{/* SCREENSHOT: JavaScript API · page builder, settings panel, or published page · desktop */}
...
## `cart-updated` event
{/* SCREENSHOT: `cart-updated` event · page builder, settings panel, or published page · desktop */}
...
The event `detail` contains `{ brandId, cart }` where `cart` is the updated array. It bubbles, so
you can listen on either `window` or `document`.

The cart is **already rendered** when the event fires: `window.efScope` and every `[[ ]]`,
`ef-text`, `template-if` and `template-foreach` binding on the page hold the new cart by the time
your handler runs. That means reading the rendered values in the handler is safe:

```js
window.addEventListener('cart-updated', () => {
  // The badge already shows the new count here.
  const badge = document.querySelector('[ef-text="efCart.count"]');
  badge.setAttribute('data-empty', parseInt(badge.textContent, 10) > 0 ? 'false' : 'true');
});
```

<Tip>
Prefer `e.detail.cart` over reading the DOM when you only need the numbers — it is the same data
without a layout read:

```js
window.addEventListener('cart-updated', (e) => {
  const count = e.detail.cart.reduce((n, item) => n + (item.quantity || 0), 0);
});
```
</Tip>
...
## Add-to-cart buttons
{/* SCREENSHOT: Add-to-cart buttons · page builder, settings panel, or published page · desktop */}
...
## Tips
{/* SCREENSHOT: Tips · page builder, settings panel, or published page · desktop */}
