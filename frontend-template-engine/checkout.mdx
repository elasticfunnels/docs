...
{/* SCREENSHOT: Order summary: single-product vs cart · code editor or rendered example · desktop */}
...
{/* SCREENSHOT: Subscription products · code editor or rendered example · desktop */}
...
| **`checkout.subscription.intro_offer`** | object \| null | `null` unless an intro offer is enabled on the product |
| **`checkout.subscription.intro_offer.price`** | number \| null | Intro price |
| **`checkout.subscription.intro_offer.period`** | number \| null | Intro period length (e.g. `2`) |
| **`checkout.subscription.intro_offer.period_unit`** | string \| null | `"day"`, `"week"`, `"month"`, or `"year"` |
| **`checkout.subscription.intro_offer.recurring_price`** | number \| null | Price charged after the intro period ends |
| **`checkout.subscription.price_steps`** | array \| null | Stepped pricing schedule; `null` when not configured |
| **`checkout.subscription.prepaid.discount_3_month`** | number \| null | Prepaid 3-month discount |
| **`checkout.subscription.prepaid.discount_6_month`** | number \| null | Prepaid 6-month discount |
| **`checkout.subscription.prepaid.discount_12_month`** | number \| null | Prepaid 12-month discount |
| **`checkout.subscription.tiers`** | array \| null | Subscription tiers; `null` when not configured |

Intro-offer copy, for example:

```html
<template-if data-condition="checkout.subscription.intro_offer">
  <p>
    [[ formatPrice(checkout.subscription.intro_offer.price) ]] for your first
    [[ checkout.subscription.intro_offer.period ]] [[ checkout.subscription.intro_offer.period_unit ]],
    then [[ formatPrice(checkout.subscription.intro_offer.recurring_price) ]].
  </p>
</template-if>
```
...
## Totals (display + raw)
{/* SCREENSHOT: Totals (display + raw) · code editor or rendered example · desktop */}
...
{/* SCREENSHOT: Form fields and validation · code editor or rendered example · desktop */}
...
## Coupon
{/* SCREENSHOT: Coupon · code editor or rendered example · desktop */}
...
## Order bumps
{/* SCREENSHOT: Order bumps · code editor or rendered example · desktop */}
...
## Payment panel
{/* SCREENSHOT: Payment panel · code editor or rendered example · desktop */}
...
{/* SCREENSHOT: Wallet (Apple Pay / Google Pay) · code editor or rendered example · desktop */}
...
Full reference: [Checkout functionality reference — `window.ef.checkout` API](/checkouts/checkout-functionality-reference#windowefcheckout-api).

---

## Summary
{/* SCREENSHOT: Summary · code editor or rendered example · desktop */}
...
- Wallet: **`checkout.wallet_visible`** (or **`checkout.wallet_available`**) for conditional content; **`ef.checkout.payWithWallet()`** for programmatic triggers; use **`<checkout-wallet force="true" />`** above the form for wallet-first layouts.
