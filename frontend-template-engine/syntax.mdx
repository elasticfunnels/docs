...
<Note>
`[[ var.something ]]` does **not** work. `var` holds brand variables, which are a server-side concept and are never placed in `window.efScope`. Print brand variables with the backend engine (`{{ var.offer_name }}`), or assign what you need into `window.efScope` yourself.
</Note>
