...
{/* SCREENSHOT: Quick reference: common pitfalls · code editor or rendered example · desktop */}
...
## Limitations
{/* SCREENSHOT: Limitations · code editor or rendered example · desktop */}
...
## Common pitfalls
{/* SCREENSHOT: Common pitfalls · code editor or rendered example · desktop */}
...
**Fix:** Add **`data-ef-cloak`** to elements that should be hidden until the template engine has run. The engine removes this attribute after processing. On ElasticFunnels-hosted pages the cloak stylesheet is already in `<head>` and hides **`[data-ef-cloak]`** plus every engine tag — **template-if**, **template-else-if**, **template-if-else**, **template-else**, **template-foreach**, **template-vars**, **template-set**, **template-partial** / **partial**. If you embed the engine elsewhere, hide that full list; a tag left out of it paints its raw content on first paint and then disappears.
</Warning>
